...
```
<br><br>
Building the clusters takes about 15 minutes. The terminal window may spit out some ``waiting``, ``disconnected``, or ``refused`` messages but as long as it doesn't return the prompt, let it do its thing as it needs to set up each instance and install a lot of software on all of them.
