...
```
<br><br>
Now we need to download our ``PEM`` key-pair file over to the instance as well:

```r
# upload pem file in same directory (swap the dashed IP with your instance's)
...
Now, let's get our clusters up and running. This will call the script to launch 3 clusters, 1 master and 2 dependents in the ``us-west`` zone. ``-k`` is your PEM name, ``-i`` is the path to the PEM file and ``-s`` is the number of instances needed:
...
Building the clusters takes about 10 minutes. The terminal window may spit out some disconnects but as long as it doesn't return the prompt, let it do its thing.
