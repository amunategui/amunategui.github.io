...
<BR><BR>
Time to create some extra observations using ``SMOTE``. We set ``perc.over = 100`` to double the quantity of positive cases, and set ``perc.under=200``, this essentially reduces our data set in size but equalizes the number of positive and negative outcomes:
...
```
<br><br>
We can check the ``outcome`` balance:
