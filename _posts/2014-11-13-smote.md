...
## 0.95446 0.04554
```
The balance between outcome variables in each split is representative of the bigger set - we're in good shape.
<BR><BR>
We train a <a href='http://topepo.github.io/caret/Bagging.html' target='_blank'>treebag</a> model using <b>caret</b> syntax on ``trainSplit`` and predict hyperthyroidism on the ``testSplit``:
