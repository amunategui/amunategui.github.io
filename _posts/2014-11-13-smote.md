...
```
<BR><BR>
The data set is still riddled with characters so we need to binarize them into numbers:
...
```
<BR><BR>
We randomly split the data set using the ``createDataPartition`` from the <b>caret</b> package into 2 equal portions:
...
## 0.95446 0.04554
```
The balance of the outcome variables in the splits are similar to the data set as a whole - we're in good shape.
<BR><BR>
We train a ``treebag`` model using <b>caret</b> syntax on ``trainSplit`` and predict hyperthyroidism on the ``testSplit``:
...
<BR><BR>
Time to create some extra observations with ``SMOTE`` and check the ``outcome`` balance afterwards. Let's start by loading the package and taking a look a the starring function ``SMOTE`` - all caps:
